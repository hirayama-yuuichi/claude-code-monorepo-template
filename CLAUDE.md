...
モノレポテンプレートの設計・管理リポジトリ。`root/` が運用されるテンプレートの実体で、
このリポジトリ自体は「`root/`」以下のテンプレートを使い、各プロジェクトを運用するためのskillやコマンドなどの・設計・計画・を行う場所。

## ファイル構成

```
root/               ← 配布するモノレポテンプレートの実体
...
重要な提案を受けるときは必ずこのループを踏む（`decisions/0001-use-提案の採用ループ.md` 参照）。
