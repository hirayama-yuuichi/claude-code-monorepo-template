...
Claude Code スキル（`/records-add`・`/critique`・`/propose`・`/package-add`・`/monorepo-init`）の正本置き場。
...
| `in-progress/` | アイデア・backlog・計画・ADR。変化が速いもの |

ADR（`in-progress/records/`）は採用後も `in-progress/` に置いたまま、ステータスフィールドで反映状態を追跡する。
`docs/` に置くのは ADR の決定を反映済みの安定情報のみ。
...
- アーキテクチャ変更は先に `in-progress/records/` に記録してから実施する
